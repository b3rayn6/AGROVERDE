...
export const generarPDFFacturaVenta = async (factura, items) => {
  ...
  // Calcular e incluir Interés por Mora si aplica
  const interes = calcularInteres(factura);
  if (interes > 0) {
    const totalPagar = factura.balance_pendiente + interes;
    
    finalY += 7;
    doc.setFontSize(10);
    doc.setTextColor(220, 38, 38); // Rojo
    doc.text('Interés Mora (3% mens.):', rightX - 60, finalY);
    doc.text(formatCurrency(interes, currency), rightX, finalY, { align: 'right' });
    
    // Mostrar equivalente en la otra moneda si es necesario
    if (currency === 'USD' && factura.tasa_cambio) {
      const interesPesos = interes * parseFloat(factura.tasa_cambio);
      finalY += 5;
      doc.setFontSize(8);
      doc.setTextColor(185, 28, 28); // Rojo oscuro
      doc.text(`(RD$ ${interesPesos.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })})`, rightX, finalY, { align: 'right' });
      doc.setFontSize(10);
      finalY += 2;
    }
    ...
    doc.setFontSize(12);
    doc.setTextColor(220, 38, 38); // Rojo
    doc.text('TOTAL A PAGAR:', rightX - 60, finalY);
    doc.text(formatCurrency(totalPagar, currency), rightX, finalY, { align: 'right' });
    doc.setTextColor(0, 0, 0); // Restaurar negro
    
    // Mostrar equivalente en la otra moneda si es necesario
    if (currency === 'USD' && factura.tasa_cambio) {
      const totalPagarPesos = totalPagar * parseFloat(factura.tasa_cambio);
      finalY += 5;
      doc.setFontSize(9);
      doc.setTextColor(185, 28, 28); // Rojo oscuro
      doc.text(`(RD$ ${totalPagarPesos.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })})`, rightX, finalY, { align: 'right' });
      doc.setTextColor(0, 0, 0);
      doc.setFontSize(10);
      finalY += 2;
    }
