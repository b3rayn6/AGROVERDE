    ...
    <body>
      <div class="header">
        ...
        <div class="header-content">
          <h1 style="font-size: 24px; margin-bottom: 5px;">AGROVERDE</h1>
          <h2 style="color: #16a34a; margin: 0 0 10px 0; font-size: 20px;">📦 ${titulo}</h2>
...
export const generarPDFRotacionInventario = (rotacionData) => {
  if (typeof window === 'undefined') {
    console.warn('PDF generation is only supported in browser environment.');
    return;
  }
  ...
  const totalValorVentas = rotacionData.reduce((sum, r) => sum + parseFloat(r.valorVentas || 0), 0);
  const totalCantidadVendida = rotacionData.reduce((sum, r) => sum + parseFloat(r.cantidadVendida || 0), 0);
    ...
    <body>
      <div class="header">
        ...
        <div class="header-content">
          ...
          <p>Total de productos analizados: ${rotacionData.length}</p>
        </div>
      </div>

      <div class="explicacion">
        <h3>📊 ¿Qué es la Rotación de Inventarios?</h3>
        <p>
          La <strong>rotación de inventarios</strong> es un indicador que nos muestra cuántas veces se vende completamente el inventario de un producto en un período determinado. 
        ...
        <p>
          Rotación = Cantidad Vendida ÷ Inventario Actual
        </p>
        <p><strong>¿Cómo interpretar los resultados?</strong></p>
        <ul>
          <li><span class="rotacion-alta">Rotación Alta (&gt;12x)</span>: Productos que se venden muy rápido. Considera aumentar el stock o la frecuencia de compra.</li>
          <li><span class="rotacion-media">Rotación Media (6-12x)</span>: Productos con venta regular. Mantén el nivel de inventario actual.</li>
          <li><span class="rotacion-baja">Rotación Baja (&lt;6x)</span>: Productos que se venden lentamente. Considera reducir el stock o revisar estrategias de venta.</li>
        </ul>
        <p>
          <strong>Días de Inventario:</strong> Indica cuántos días durará el inventario actual con el ritmo de venta actual. 
          Si es muy bajo, considera aumentar el stock. Si es muy alto, podrías tener productos estancados.
        </p>
      </div>
      ...
      <div class="resumen">
        <div class="resumen-card">
          <h3>Total Productos</h3>
          <p>${rotacionData.length}</p>
        </div>
        <div class="resumen-card">
          <h3>Cantidad Total Vendida</h3>
          <p>${totalCantidadVendida.toFixed(2)}</p>
        </div>
        <div class="resumen-card">
          <h3>Valor Total Ventas</h3>
          <p>${formatCurrency(totalValorVentas)}</p>
        </div>
      </div>
      ...
      <table>
        ...
        <tbody>
          ${rotacionData.map(r => {
            const rotacionNum = typeof r.rotacion === 'string' && r.rotacion === '∞' ? Infinity : parseFloat(r.rotacion || 0);
            let claseRotacion = '';
            if (rotacionNum === Infinity) {
              claseRotacion = 'rotacion-alta';
            } else if (rotacionNum > 12) {
              claseRotacion = 'rotacion-alta';
            } else if (rotacionNum > 6) {
              claseRotacion = 'rotacion-media';
            } else {
              claseRotacion = 'rotacion-baja';
            }
            ...
            return `
              <tr>
                ...
                <td>${r.nombre || '-'}</td>
                <td>${r.categoria || '-'}</td>
                <td>${r.cantidadVendida.toFixed(2)} ${r.unidad_medida || ''}</td>
                <td>${formatCurrency(r.valorVentas)}</td>
                <td>${r.inventarioActual.toFixed(2)} ${r.unidad_medida || ''}</td>
                <td class="${claseRotacion}">${typeof r.rotacion === 'string' && r.rotacion === '∞' ? '∞' : parseFloat(r.rotacion || 0).toFixed(2)}x</td>
                <td>${r.diasInventario !== null && r.diasInventario !== undefined ? r.diasInventario.toFixed(1) + ' días' : 'N/A'}</td>
                <td>${r.numeroVentas || 0}</td>
              </tr>
            `;
          }).join('')}
          <tr class="total-row">
            <td colspan="3" style="text-align: right;">TOTALES:</td>
            <td>${totalCantidadVendida.toFixed(2)}</td>
            <td>${formatCurrency(totalValorVentas)}</td>
            <td colspan="4"></td>
          </tr>
        </tbody>
      </table>
      ...
      <div class="footer">
        <p>Sistema de Gestión de Arroz - Reporte de Rotación de Inventarios generado automáticamente</p>
      </div>
    </body>
    </html>
  `;
