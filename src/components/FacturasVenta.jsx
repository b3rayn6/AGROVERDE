...
export default function FacturasVenta() {
  ...
  const generarPDF = async () => {
    ...
    generarPDFFacturasVenta(facturasFiltradas);
  };
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      if (facturasRes.error) console.error('Error facturas:', facturasRes.error);
      if (clientesRes.error) console.error('Error clientes:', clientesRes.error);
      if (mercanciasRes.error) console.error('Error mercancias:', mercanciasRes.error);
      ...
      alert('Error de conexión o datos: ' + (error.message || 'Error desconocido'));
    }
    setLoading(false);
  };
  ...
  const agregarItem = () => {
    ...
    // En modo edición, calcular stock disponible considerando items que ya están en la factura
    // (que se devolverán al inventario al guardar)
    let stockDisponible = mercancia.stock_actual;
    if (modoEdicion) {
      // Sumar la cantidad de este producto que ya está en la factura (se devolverá al inventario)
      const itemExistente = nuevaFactura.items.find(item => item.mercancia_id === parseInt(nuevoItem.mercancia_id));
      if (itemExistente) {
        stockDisponible += itemExistente.cantidad;
      }
    }

    // Verificar si ya existe este producto en la lista de items
    const itemYaExiste = nuevaFactura.items.some(item => item.mercancia_id === parseInt(nuevoItem.mercancia_id));
    if (itemYaExiste && !modoEdicion) {
      alert('Este producto ya está en la factura. Si desea modificar la cantidad, elimínelo primero y vuelva a agregarlo.');
      return;
    }

    if (parseFloat(nuevoItem.cantidad) > stockDisponible) {
      alert(`Stock insuficiente. Disponible: ${stockDisponible} ${mercancia.unidad_medida}`);
      return;
    }

    const subtotal = parseFloat(nuevoItem.cantidad) * parseFloat(nuevoItem.precio_unitario);

    // Si el producto ya existe en modo edición, actualizar cantidad en lugar de agregar duplicado
    if (modoEdicion && itemYaExiste) {
      const itemIndex = nuevaFactura.items.findIndex(item => item.mercancia_id === parseInt(nuevoItem.mercancia_id));
      const nuevaCantidad = parseFloat(nuevoItem.cantidad);
      const nuevoSubtotal = nuevaCantidad * parseFloat(nuevoItem.precio_unitario);
  ...
  const actualizarItem = (index, campo, valor) => {
    ...
    if (campo === 'cantidad') {
      ...
      const mercancia = mercancias.find(m => m.id === item.mercancia_id);
      if (mercancia) {
        let stockDisponible = mercancia.stock_actual;
        
        // En modo edición, considerar que los items anteriores se devolverán al inventario
        if (modoEdicion) {
          const itemAnterior = nuevaFactura.items.find((it, idx) => 
            it.mercancia_id === item.mercancia_id && idx !== index
          );
          // Si hay otro item del mismo producto, no sumar su cantidad
          // Solo considerar el stock actual más la cantidad actual de este item
          stockDisponible = mercancia.stock_actual + item.cantidad;
        } else {
          // En modo creación, verificar stock disponible menos otros items del mismo producto
          const otrosItems = nuevaFactura.items.filter((it, idx) => 
            it.mercancia_id === item.mercancia_id && idx !== index
          );
          const cantidadEnOtrosItems = otrosItems.reduce((sum, it) => sum + it.cantidad, 0);
          stockDisponible = mercancia.stock_actual - cantidadEnOtrosItems;
        }
    ...
    try {
      ...
      if (modoEdicion) {
        ...
        const { data: facturaAnterior } = await supabase
          .from('facturas_venta')
          .select('balance_pendiente, cliente_id, monto_pagado')
        ...
        // Si la factura anterior tenía balance pendiente, restarlo del cliente
        // NOTA: No eliminamos de cuentas_por_cobrar para mantener el historial
        ...
        for (const item of nuevaFactura.items) {
          ...
          // Buscar si este producto estaba en los items anteriores
          const itemAnterior = itemsAnteriores?.find(ia => ia.mercancia_id === item.mercancia_id);
          const stockDisponible = mercancia.stock_actual + (itemAnterior?.cantidad || 0);

          if (item.cantidad > stockDisponible) {
            alert(`Stock insuficiente para ${mercancia.nombre}. Disponible: ${stockDisponible} (incluyendo productos a devolver)`);
            setLoading(false);
            return;
          }
        }

        // Determinar estado según tipo de venta y pagos previos
        const esContado = nuevaFactura.tipo_venta === 'contado';
        
        // Calcular montos conservando pagos previos si es a crédito
        let montoPagadoFinal = 0;
        let balancePendienteFinal = 0;
        let estadoFinal = 'pendiente';
        
        if (esContado) {
          montoPagadoFinal = total;
          balancePendienteFinal = 0;
          estadoFinal = 'pagada';
        } else {
          // Si es a crédito, mantener lo que ya se había pagado
          const pagadoPrevio = facturaAnterior?.monto_pagado || 0;
          montoPagadoFinal = pagadoPrevio;
          balancePendienteFinal = Math.max(0, total - pagadoPrevio);
          estadoFinal = balancePendienteFinal === 0 ? 'pagada' : (montoPagadoFinal > 0 ? 'parcial' : 'pendiente');
        }
        ...
        await supabase
          ...
          .update({
            ...
            itbis,
            total,
            balance_pendiente: balancePendienteFinal,
        ...
        // Si es a crédito, actualizar balance del cliente con el nuevo total y registrar/actualizar en cuentas_por_cobrar
        if (!esContado) {
          const { data: cliente } = await supabase
            .from('clientes')
          ...
          await supabase
            .from('clientes')
            .update({ balance_pendiente: (cliente.balance_pendiente || 0) + balancePendienteFinal })
            .eq('id', nuevaFactura.cliente_id);

          // Verificar si ya existe un registro en cuentas_por_cobrar
          ...
          const datosCuenta = {
            ...
            monto_total: total,
            monto_pendiente: balancePendienteFinal,
            fecha_emision: nuevaFactura.fecha,
            fecha_vencimiento: null,
            estado: estadoFinal === 'pagada' ? 'Pagada' : 'Pendiente',
            divisa: nuevaFactura.divisa
          };
  ...
  const calcularInteres = (factura) => {
    if (!factura || factura.balance_pendiente <= 0) return 0;
    ...
    // Si la fecha es futura, no hay interés
    ...
    if (diffDays > 30) {
      const diasMora = diffDays - 30;
      // Interés simple: 3% mensual sobre el saldo pendiente
      // Asumiendo 30 días por mes para el cálculo diario: 3% / 30 = 0.1% diario
      const tasaDiaria = 0.03 / 30;
      const interes = factura.balance_pendiente * tasaDiaria * diasMora;
      return interes;
    }
  ...
  const facturasFiltradas = facturas.filter(f => {
    const cumpleBusqueda = f.numero_factura.toLowerCase().includes(busqueda.toLowerCase()) ||
      f.clientes?.nombre.toLowerCase().includes(busqueda.toLowerCase());
    
    const cumpleTipo = filtroTipo === 'todos' || f.tipo_venta === filtroTipo;

    let cumpleFecha = true;
    if (filtroFecha.desde) {
      cumpleFecha = cumpleFecha && new Date(f.fecha) >= new Date(filtroFecha.desde);
    }
    if (filtroFecha.hasta) {
      cumpleFecha = cumpleFecha && new Date(f.fecha) <= new Date(filtroFecha.hasta);
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="relative md:col-span-2">
              ...
              <input
                ...
                className="w-full pl-10 sm:pl-12 pr-4 py-2 sm:py-3 border-2 border-gray-100 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all duration-200 bg-gray-50 focus:bg-white text-gray-900 text-sm sm:text-base"
              />
            </div>
            ...
            <div className="flex gap-2">
               <input
                ...
                className="w-full px-3 py-2 sm:py-3 border-2 border-gray-100 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all duration-200 bg-gray-50 focus:bg-white text-gray-900 text-sm"
                placeholder="Desde"
              />
              <input
                type="date"
            ...
            <div className="relative">
              <select
                ...
                className="w-full px-4 py-2 sm:py-3 border-2 border-gray-100 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all duration-200 bg-gray-50 focus:bg-white text-gray-900 text-sm sm:text-base appearance-none"
              >
                ...
                <div className="border-t pt-4 sm:pt-6 mb-4 sm:mb-6">
                  <div className="flex items-center justify-between mb-3 sm:mb-4">
                    <h3 className="text-base sm:text-lg font-semibold">
                      {modoEdicion ? 'Agregar o Modificar Productos' : 'Agregar Productos'}
                    </h3>
                    {modoEdicion && (
                      <span className="text-xs sm:text-sm text-blue-600 bg-blue-50 px-2 py-1 rounded-md">
                        💡 Puede agregar nuevos productos o eliminar existentes
                      </span>
                    )}
                  </div>
                    ...
                    <div className="sm:col-span-2">
                      ...
                      >
                        ...
                        {mercancias
                          ...
                          .map(m => {
                            // En modo edición, calcular stock disponible considerando items que ya están en la factura
                            let stockDisponible = m.stock_actual;
                            if (modoEdicion) {
                              const itemExistente = nuevaFactura.items.find(item => item.mercancia_id === m.id);
                              if (itemExistente) {
                                stockDisponible += itemExistente.cantidad;
                              }
                            }
                            return (
                              <option key={m.id} value={m.id}>
                                {m.codigo ? `[${m.codigo}] ` : ''}{m.nombre} - Stock: {stockDisponible} {m.unidad_medida}
                                {modoEdicion && nuevaFactura.items.some(item => item.mercancia_id === m.id) && ' (incl. en factura)'}
                              </option>
                            );
                          })}
                      </select>
                  ...
                  <button
                    onClick={agregarItem}
                    className="w-full sm:w-auto bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 flex items-center justify-center gap-2 text-sm sm:text-base shadow-md hover:shadow-lg transition-all"
                  >
                    <Plus size={18} className="sm:w-5 sm:h-5" />
                    {modoEdicion ? 'Agregar o Actualizar Producto' : 'Agregar Item'}
                  </button>
                </div>
                ...
                {nuevaFactura.items.length > 0 && (
                  <div className="border-t pt-4 sm:pt-6 mb-4 sm:mb-6">
                    <div className="flex items-center justify-between mb-3 sm:mb-4">
                      <div>
                        <h3 className="text-base sm:text-lg font-semibold">
                          {modoEdicion ? 'Productos de la Factura (Editable)' : 'Items de la Factura'}
                        </h3>
                        <p className="text-xs text-gray-500 mt-1">
                          ✏️ Puede editar cantidad y precio directamente en la tabla
                        </p>
                      </div>
                      <span className="text-xs text-gray-500">
                        {nuevaFactura.items.length} {nuevaFactura.items.length === 1 ? 'producto' : 'productos'}
                      </span>
                    </div>
                        ...
                        <tbody className="divide-y divide-gray-200">
                          {nuevaFactura.items.map((item, index) => (
                            <tr key={index} className="hover:bg-gray-50">
                              <td className="px-2 sm:px-4 py-2 text-xs sm:text-sm font-medium">{item.mercancia_nombre}</td>
                              <td className="px-2 sm:px-4 py-2 text-right">
                                <input
                                  ...
                                  onBlur={(e) => {
                                    ...
                                    if (!valor || valor <= 0) {
                                      actualizarItem(index, 'cantidad', item.cantidad);
                                    }
                                  }}
                                  className="w-20 sm:w-24 px-2 py-1.5 text-xs sm:text-sm text-right border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 hover:border-gray-400 transition-colors bg-white"
                                  min="0"
                                  step="0.01"
                              ...
                              <td className="px-2 sm:px-4 py-2 text-right">
                                <div className="flex items-center justify-end gap-1.5">
                                  <span className="text-xs text-gray-500 font-medium">{nuevaFactura.divisa}</span>
                                  <input
                                    type="number"
                                    ...
                                    onBlur={(e) => {
                                      const valor = parseFloat(e.target.value);
                                      if (!valor || valor < 0) {
                                        actualizarItem(index, 'precio_unitario', item.precio_unitario);
                                      }
                                    }}
                                    className="w-24 sm:w-32 px-2 py-1.5 text-xs sm:text-sm text-right border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 hover:border-gray-400 transition-colors bg-white"
                                    min="0"
                                    step="0.01"
                                    title="Haga clic para editar el precio unitario"
                                  />
                                </div>
                              </td>
                              <td className="px-2 sm:px-4 py-2 text-xs sm:text-sm text-right font-medium">
                                {formatCurrency(item.subtotal, nuevaFactura.divisa)}
                              </td>
                              <td className="px-2 sm:px-4 py-2 text-center">
                                <button
                                  onClick={() => {
                                    if (confirm(`¿Está seguro de eliminar "${item.mercancia_nombre}" de la factura?`)) {
                                      eliminarItem(index);
                                    }
                                  }}
                                  className="text-red-600 hover:text-red-800 bg-red-50 hover:bg-red-100 px-2 py-1 rounded transition-all"
                ...
                <div className="border-t pt-4">
                  <div className="flex justify-end">
                    <div className="w-full sm:w-64">
                      ...
                      {/* Mostrar Interés si aplica */}
                      {calcularInteres(facturaSeleccionada) > 0 && (
                        <>
                          <div className="flex justify-between mb-2 text-sm sm:text-base bg-red-50 p-1 rounded">
                            <span className="text-red-600 font-medium">Interés Mora (3% mens.):</span>
                            <span className="font-bold text-red-600">
                              {formatCurrency(calcularInteres(facturaSeleccionada), facturaSeleccionada.divisa || 'DOP')}
                            </span>
                          </div>
                          <div className="flex justify-between text-base sm:text-lg font-bold border-t border-red-200 pt-2 mt-2">
                            <span>Total a Pagar:</span>
                            <span className="text-red-600">
                              {formatCurrency(facturaSeleccionada.balance_pendiente + calcularInteres(facturaSeleccionada), facturaSeleccionada.divisa || 'DOP')}
                            </span>
                          </div>
                        </>
                      )}
                    ...
                    <div>
                      ...
                      {calcularInteres(facturaSeleccionada) > 0 && (
                        <p className="text-xs font-bold text-red-600 mt-1">
                          + {formatCurrency(calcularInteres(facturaSeleccionada), facturaSeleccionada.divisa || 'DOP')} Mora
                        </p>
                      )}
                    </div>
                  </div>
