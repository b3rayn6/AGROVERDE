...
export default function CuentasPorCobrar() {
  ...
  const sincronizarFacturas = async () => {
    try {
      ...
      // 1. Obtener todas las facturas de venta a crédito
      const { data: facturas, error: errorFacturas } = await supabase
        .from('facturas_venta')
        .select('*, clientes(nombre, cedula)')
        .neq('tipo_venta', 'contado');
      ...
      // Mapa por referencia (acepta cualquier tipo)
      const mapaExistentes = new Map();
      for (const c of (cuentasExistentes || [])) {
        if (!mapaExistentes.has(c.referencia)) {
          mapaExistentes.set(c.referencia, c);
        }
      }
      ...
      if (nuevasCuentas.length > 0) {
        console.log(`Insertando ${nuevasCuentas.length} facturas faltantes...`);
              ...
              <button
                onClick={async () => {
                  ...
                  for (const c of (todasCuentas || [])) {
                    if (!grupos[c.referencia]) grupos[c.referencia] = [];
                    grupos[c.referencia].push(c);
                  }
                  let eliminados = 0;
