...
import { Truck, Search, Edit2, Save, X, FileText, Filter } from 'lucide-react';
...
export default function RegistroFlete({ user }) {
  ...
  // Check if user can edit fletes (only admin@admin.com)
  const puedeEditar = () => {
  ...
  const fletesFiltrados = fletes.filter(flete => {
    ...
    return matchSearch && matchChofer && matchFactoria;
  });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-50 p-4">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Formulario - Solo visible para usuarios con permisos de edición */}
        {puedeEditar() && (
          <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
        ...
        <div className="bg-white rounded-xl shadow-lg mb-6 overflow-hidden">
          <div className="flex border-b border-gray-200">
            <button
              className={`flex-1 py-4 px-6 text-center font-semibold transition-colors ${activeTab === 'individual' ? 'bg-green-50 text-green-700 border-b-2 border-green-600' : 'text-gray-500 hover:bg-gray-50'}`}
              onClick={() => setActiveTab('individual')}
            >
              Fletes Individuales
            </button>
            <button
              className={`flex-1 py-4 px-6 text-center font-semibold transition-colors ${activeTab === 'agrupado' ? 'bg-green-50 text-green-700 border-b-2 border-green-600' : 'text-gray-500 hover:bg-gray-50'}`}
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-4">
            ...
            {activeTab === 'individual' ? (
              <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
                ...
                {(choferFiltro || factoriaFiltro || searchTerm) && (
                  <button
                    onClick={() => {
                      setChoferFiltro('');
                      setFactoriaFiltro('');
              ...
              <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto items-center">
                ...
                {(choferResumenFiltro || factoriaResumenFiltro) && (
                  <button
                    onClick={() => {
                      setChoferResumenFiltro('');
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {(choferResumenFiltro ? choferesAgrupados.filter(c => c.nombre === choferResumenFiltro) : choferesAgrupados).map((chofer) => (
                      <React.Fragment key={chofer.nombre}>
                        ...
                        {choferResumenFiltro && (
                          <tr>
                            <td colSpan="7" className="p-4 bg-green-50/50 border-t border-gray-200">
