  ...
  const renderPesadasModule = () => (
    <div className="w-full max-w-full overflow-x-hidden">
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6 max-w-full">
        ...
        <div className="flex-1 relative min-w-0">
          <div className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 font-semibold text-sm">N°</div>
          <input
            type="text"
          ...
          <table className="w-full min-w-[800px]">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Kg Bruto</th>
              ...
              {loading ? (
                ...
                filteredPesadas.map((pesada) => (
                  <tr key={pesada.id} className={`hover:bg-gray-50 transition-colors ${selectedPesadas.includes(pesada.id) ? 'bg-green-50' : ''}`}>
                    ...
                    <td className="px-6 py-4 text-sm text-gray-600">{parseFloat(pesada.kilos_neto || 0).toFixed(2)}</td>
            ...
            {filteredPesadas.length > 0 && (
              <tfoot className="bg-gray-100 border-t-2 border-gray-300">
                <tr>
                  <td colSpan="4" className="px-6 py-4 text-sm font-bold text-gray-800 text-right uppercase">Totales:</td>
                  <td className="px-6 py-4 text-sm font-bold text-gray-800">{formatNumber(totalSacos, 0)}</td>
