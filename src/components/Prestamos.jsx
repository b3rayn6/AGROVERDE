...
import { DollarSign, Plus, X, Download, Calendar, TrendingUp, AlertCircle, Edit, Eye, Search, Filter } from 'lucide-react';
...
export default function Prestamos() {
  ...
  const [showDesgloseGeneralModal, setShowDesgloseGeneralModal] = useState(false);
  ...
  const prestamosFiltrados = useMemo(() => {
    return prestamos.filter(p => {
      ...
      const cumpleBusqueda =
        (p.nombre_cliente?.toLowerCase() || '').includes(busquedaLower) ||
        (p.cedula_cliente || '').includes(busquedaLower) ||
        (p.id?.toString() || '').includes(busquedaLower);
      
      const fechaPrestamo = new Date(p.fecha_prestamo);
      const cumpleFechaInicio = !filtros.fechaInicio || fechaPrestamo >= new Date(filtros.fechaInicio);
      const cumpleFechaFin = !filtros.fechaFin || fechaPrestamo <= new Date(filtros.fechaFin);
      const cumpleEstado = !filtros.estado || p.estado === filtros.estado;

      return cumpleBusqueda && cumpleFechaInicio && cumpleFechaFin && cumpleEstado;
    });
  }, [prestamos, filtros]);
      ...
      if (facturaSeleccionada) {
        ...
        const { error: errorCxC } = await supabase
          ...
          .update({
            ...
            estado: nuevoBalanceFactura <= 0 ? 'Pagada' : 'Pendiente'
          })
  ...
  const abrirDesglose = async (prestamo) => {
    setPrestamoSeleccionado(prestamo);
  ...
  const generarPDFGeneral = async (prestamosConPagos = []) => {
    if (typeof window === 'undefined') return;
    ...
    const doc = new jsPDF();
    const prestamosData = prestamosConPagos.length > 0 ? prestamosConPagos : prestamos;
    ...
    doc.setFontSize(16);
    doc.setFont(undefined, 'bold');
    doc.text('REPORTE GENERAL DE FINANCIAMIENTOS', 105, 45, { align: 'center' });

    // Resumen general
    const totalPrestado = prestamosData.reduce((sum, p) => sum + parseFloat(p.monto_prestado || 0), 0);
    const totalInteres = prestamosData.reduce((sum, p) => sum + parseFloat(p.interes_generado || 0), 0);
    const totalPagar = prestamosData.reduce((sum, p) => sum + parseFloat(p.total_pagar || 0), 0);
    const balancePendiente = prestamosData.reduce((sum, p) => sum + parseFloat(p.balance_pendiente || 0), 0);
    ...
    doc.setTextColor(255, 255, 255);
    doc.text('RESUMEN GENERAL', 105, 60, { align: 'center' });

    doc.setTextColor(0, 0, 0);
    doc.setFont(undefined, 'normal');
    doc.text(`Total Prestado: ${formatMoney(totalPrestado)}`, 20, 70);
    doc.text(`Total Interés: ${formatMoney(totalInteres)}`, 20, 77);
    doc.text(`Total a Pagar: ${formatMoney(totalPagar)}`, 20, 84);
    doc.setFont(undefined, 'bold');
    doc.text(`Balance Pendiente: ${formatMoney(balancePendiente)}`, 20, 91);
    doc.text(`Cantidad de Financiamientos: ${prestamosData.length}`, 20, 98);

    // Tabla de préstamos
    const tableData = prestamosData.map(p => [
    ...
    doc.save(`financiamientos_general_${new Date().toISOString().split('T')[0]}.pdf`);
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => setShowDesgloseGeneralModal(true)}
              ...
              <button
                onClick={async () => {
                  ...
                  const prestamosConPagos = prestamosFiltrados.map(p => {
                    const pagosPrestamo = pagosData?.filter(pa => pa.financiamiento_id === p.id) || [];
                    return { ...p, pagos: pagosPrestamo };
                  });
                  
                  generarPDFGeneral(prestamosConPagos);
                }}
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm mb-6">
          <div className="flex flex-col md:flex-row gap-4 items-end">
            ...
            <button
              onClick={() => setFiltros({ busqueda: '', fechaInicio: '', fechaFin: '', estado: '' })}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white p-4 rounded-lg shadow-sm">
            ...
            <p className="text-2xl font-bold text-gray-800">
              {formatMoney(prestamosFiltrados.reduce((sum, p) => sum + parseFloat(p.monto_prestado), 0))}
            </p>
          </div>
          <div className="bg-white p-4 rounded-lg shadow-sm">
            <p className="text-gray-600 text-sm">Total a Cobrar</p>
            <p className="text-2xl font-bold text-green-600">
              {formatMoney(prestamosFiltrados.reduce((sum, p) => sum + parseFloat(p.total_pagar), 0))}
            </p>
          </div>
          <div className="bg-white p-4 rounded-lg shadow-sm">
            <p className="text-gray-600 text-sm">Balance Pendiente</p>
            <p className="text-2xl font-bold text-blue-600">
              {formatMoney(prestamosFiltrados.reduce((sum, p) => sum + parseFloat(p.balance_pendiente), 0))}
            </p>
          </div>
          <div className="bg-white p-4 rounded-lg shadow-sm">
                  ...
                  <tr key={prestamo.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2 flex-wrap">
                        <button
                          onClick={() => abrirDesglose(prestamo)}
                          className="bg-indigo-600 text-white px-3 py-1 rounded hover:bg-indigo-700 text-xs font-medium flex items-center gap-1"
                          title="Ver Desglose"
                        >
                        ...
                        <button
                          onClick={async () => {
                            const { data } = await supabase
                              ...
                              .eq('financiamiento_id', prestamo.id)
                              .order('fecha_pago', { ascending: false });
                            generatePrestamoPDF(prestamo, data || []);
                          }}
                          className="bg-green-600 text-white px-3 py-1 rounded hover:bg-green-700 text-xs font-medium flex items-center gap-1"
                          title="Descargar PDF"
                        >
                      ...
                      <div className="flex gap-4">
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input
                            ...
                            name="divisa"
                            value="DOP"
                            checked={formData.divisa === 'DOP'}
                            onChange={(e) => setFormData({...formData, divisa: e.target.value})}
                            className="w-4 h-4 text-green-600 focus:ring-green-500"
                          />
                          <span className="text-gray-700">Pesos (DOP)</span>
                        </label>
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input
                            type="radio"
                            name="divisa"
                            value="USD"
                            checked={formData.divisa === 'USD'}
                            onChange={(e) => setFormData({...formData, divisa: e.target.value})}
                            className="w-4 h-4 text-green-600 focus:ring-green-500"
                          />
                          <span className="text-gray-700">Dólares (USD)</span>
                        </label>
                      </div>
                    </div>
        ...
        {/* Modal Desglose */}
        {showDesgloseModal && prestamoSeleccionado && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-lg max-w-5xl w-full max-h-[90vh] overflow-y-auto">
              <div className="p-6">
                <div className="flex justify-between items-center mb-6">
                  <div>
                    <h2 className="text-2xl font-bold text-gray-800">Desglose de Financiamiento</h2>
                    <p className="text-gray-600">{prestamoSeleccionado.nombre_cliente}</p>
                  </div>
                  <button onClick={() => setShowDesgloseModal(false)} className="text-gray-500 hover:text-gray-700">
                    <X className="w-6 h-6" />
                  </button>
                </div>
                ...
                <div className="bg-gray-50 p-4 rounded-lg mb-6">
                  <h3 className="font-bold text-gray-800 mb-4">Información del Financiamiento</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <p className="text-sm text-gray-600">Cliente:</p>
                      <p className="font-medium text-gray-800">{prestamoSeleccionado.nombre_cliente}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">Cédula:</p>
                      <p className="font-medium text-gray-800">{prestamoSeleccionado.cedula_cliente}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">Fecha del Préstamo:</p>
                      <p className="font-medium text-gray-800">{new Date(prestamoSeleccionado.fecha_prestamo).toLocaleDateString()}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">Fecha de Vencimiento:</p>
                      <p className="font-medium text-gray-800">{new Date(prestamoSeleccionado.fecha_vencimiento).toLocaleDateString()}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">Plazo:</p>
                      <p className="font-medium text-gray-800">{prestamoSeleccionado.plazo_meses} meses</p>
                    </div>
                    <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                  <div className="bg-blue-50 p-4 rounded-lg">
                    ...
                    <p className="text-xl font-bold text-blue-600">{formatMoney(prestamoSeleccionado.monto_prestado)}</p>
                  </div>
                  <div className="bg-orange-50 p-4 rounded-lg">
                    <p className="text-xs text-gray-600 mb-1">Tasa de Interés</p>
                    <p className="text-xl font-bold text-orange-600">{prestamoSeleccionado.tasa_interes}%</p>
                  </div>
                  <div className="bg-purple-50 p-4 rounded-lg">
                    <p className="text-xs text-gray-600 mb-1">Interés Generado</p>
                    <p className="text-xl font-bold text-purple-600">{formatMoney(prestamoSeleccionado.interes_generado)}</p>
                  </div>
                  <div className="bg-green-50 p-4 rounded-lg">
                    <p className="text-xs text-gray-600 mb-1">Total a Pagar</p>
                    <p className="text-xl font-bold text-green-600">{formatMoney(prestamoSeleccionado.total_pagar)}</p>
                  </div>
                </div>

                {/* Cálculos Actualizados */}
                <div className="bg-indigo-50 p-4 rounded-lg mb-6">
                  <h3 className="font-bold text-gray-800 mb-4">Cálculos Actualizados al Día de Hoy</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div>
                      <p className="text-xs text-gray-600 mb-1">Días Transcurridos</p>
                      <p className="text-lg font-bold text-indigo-600">{prestamoSeleccionado.diasTranscurridos || 0} días</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-600 mb-1">Interés Diario</p>
                      <p className="text-lg font-bold text-indigo-600">{formatMoney(prestamoSeleccionado.interesDiario || 0)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-600 mb-1">Interés Acumulado</p>
                      <p className="text-lg font-bold text-indigo-600">{formatMoney(prestamoSeleccionado.interesAcumuladoActual || 0)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-600 mb-1">Total Actualizado</p>
                      <p className="text-lg font-bold text-indigo-600">{formatMoney(prestamoSeleccionado.totalActualizado || prestamoSeleccionado.total_pagar)}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 p-4 rounded-lg mb-6 grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-purple-600">
                      {((((parseFloat(prestamoSeleccionado.total_pagar) - parseFloat(prestamoSeleccionado.balance_pendiente)) / parseFloat(prestamoSeleccionado.total_pagar)) * 100) || 0).toFixed(2)}%
                    </p>
                  </div>
                </div>
                ...
                {/* Historial de Pagos */}
                <div>
                  <h3 className="font-bold text-gray-800 mb-4">Historial de Pagos ({pagosPrestamo.length})</h3>
                  {pagosPrestamo.length === 0 ? (
                    <p className="text-gray-500 text-center py-8 bg-gray-50 rounded-lg">No hay pagos registrados</p>
                  ) : (
                    <div className="overflow-x-auto">
                      <table className="w-full">
                        <thead className="bg-gray-50 border-b">
                          <tr>
                            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Fecha</th>
                            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Monto Pagado</th>
                            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Balance Después del Pago</th>
                            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Notas</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-200">
                ...
                {prestamoSeleccionado.notas && (
                  <div className="mt-6 bg-yellow-50 p-4 rounded-lg">
                    ...
                    <p className="text-sm text-gray-700">{prestamoSeleccionado.notas}</p>
                  </div>
                )}
                  ...
                  <button
                    onClick={async () => {
                      const { data } = await supabase
                        .from('pagos_financiamientos')
                        .select('*')
                        .eq('financiamiento_id', prestamoSeleccionado.id)
                        .order('fecha_pago', { ascending: false });
                      generatePrestamoPDF(prestamoSeleccionado, data || []);
                    }}
                    className="bg-green-600 text-white px-6 py-2 rounded-lg hover:bg-green-700 flex items-center gap-2"
                  >
                    <Download className="w-4 h-4" />
        ...
        {/* Modal Desglose General */}
        {showDesgloseGeneralModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-lg max-w-7xl w-full max-h-[90vh] overflow-y-auto">
              <div className="p-6">
                <div className="flex justify-between items-center mb-6">
                  <div>
                    <h2 className="text-2xl font-bold text-gray-800">Desglose General de Financiamientos</h2>
                    <p className="text-gray-600">Vista completa de todos los financiamientos</p>
                  </div>
                  <button onClick={() => setShowDesgloseGeneralModal(false)} className="text-gray-500 hover:text-gray-700">
                    <X className="w-6 h-6" />
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                  <div className="bg-blue-50 p-4 rounded-lg">
                    ...
                    <p className="text-2xl font-bold text-blue-600">
                      {formatMoney(prestamos.reduce((sum, p) => sum + parseFloat(p.monto_prestado || 0), 0))}
                    </p>
                  </div>
                  <div className="bg-orange-50 p-4 rounded-lg">
                    <p className="text-xs text-gray-600 mb-1">Total Interés</p>
                    <p className="text-2xl font-bold text-orange-600">
                      {formatMoney(prestamos.reduce((sum, p) => sum + parseFloat(p.interes_generado || 0), 0))}
                    </p>
                  </div>
                  <div className="bg-green-50 p-4 rounded-lg">
                    <p className="text-xs text-gray-600 mb-1">Total a Pagar</p>
                    <p className="text-2xl font-bold text-green-600">
                      {formatMoney(prestamos.reduce((sum, p) => sum + parseFloat(p.total_pagar || 0), 0))}
                    </p>
                  </div>
                  <div className="bg-purple-50 p-4 rounded-lg">
                    <p className="text-xs text-gray-600 mb-1">Balance Pendiente</p>
                    <p className="text-2xl font-bold text-purple-600">
                      {formatMoney(prestamos.reduce((sum, p) => sum + parseFloat(p.balance_pendiente || 0), 0))}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                  <div className="bg-indigo-50 p-4 rounded-lg">
                    <p className="text-xs text-gray-600 mb-1">Total Financiamientos</p>
                    <p className="text-xl font-bold text-indigo-600">{prestamos.length}</p>
                  </div>
                  <div className="bg-green-50 p-4 rounded-lg">
                    <p className="text-xs text-gray-600 mb-1">Financiamientos Activos</p>
                    <p className="text-xl font-bold text-green-600">
                      {prestamos.filter(p => p.estado === 'Activo').length}
                    </p>
                  </div>
                  <div className="bg-red-50 p-4 rounded-lg">
                    <p className="text-xs text-gray-600 mb-1">Financiamientos Atrasados</p>
                    <p className="text-xl font-bold text-red-600">
                      {prestamos.filter(p => p.estado === 'Atrasado').length}
                    </p>
                  </div>
                </div>
                ...
                {/* Tabla de Financiamientos */}
                <div className="mb-6">
                  <h3 className="font-bold text-gray-800 mb-4">Lista de Financiamientos ({prestamos.length})</h3>
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead className="bg-gray-50 border-b">
                        <tr>
                        ...
                        {prestamos.map((prestamo) => (
                          <tr key={prestamo.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-4 py-3 text-sm font-medium text-gray-900">
                              {formatMoney(prestamo.monto_prestado)}
                            </td>
                            <td className="px-4 py-3 text-sm text-gray-700">{prestamo.tasa_interes}%</td>
                            <td className="px-4 py-3 text-sm font-medium text-orange-600">
                              {formatMoney(prestamo.interes_generado)}
                            </td>
                            <td className="px-4 py-3 text-sm font-medium text-green-600">
                              {formatMoney(prestamo.total_pagar)}
                            </td>
                            <td className="px-4 py-3 text-sm font-medium text-blue-600">
                              {formatMoney(prestamo.balance_pendiente)}
                            </td>
                            <td className="px-4 py-3">
                              <span className={`px-2 py-1 text-xs font-medium rounded-full ${getEstadoColor(prestamo.estado)}`}>
                                {prestamo.estado}
                            ...
                            <td className="px-4 py-3 text-sm font-bold text-purple-600">
                              {prestamo.diasTranscurridos || 0} días
                            </td>
                          </tr>
                        ))}
                      </tbody>
                ...
                <div className="mt-6 flex justify-end gap-3">
                  <button
                    onClick={async () => {
                      ...
                      const prestamosConPagos = prestamos.map(p => {
                        const pagosPrestamo = pagosData?.filter(pa => pa.financiamiento_id === p.id) || [];
                        return { ...p, pagos: pagosPrestamo };
                      });
                  ...
                  <button
                    onClick={() => setShowDesgloseGeneralModal(false)}
                    className="bg-gray-600 text-white px-6 py-2 rounded-lg hover:bg-gray-700"
                  >
                    Cerrar
                  </button>
