...
export default function VentasDiarias() {
  ...
  const generarPDF = async () => {
    ...
    generarPDFVentasDiarias(ventasFiltradas);
  };
  ...
  const ventasFiltradas = ventas.filter(v => {
    const cumpleBusqueda = v.numero_venta?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    (v.cliente_nombre || '').toLowerCase().includes(searchTerm.toLowerCase());
    
    const cumpleTipo = filtroTipo === 'todos' || v.tipo_venta === filtroTipo;

    let cumpleFecha = true;
    if (filtroFecha.desde) {
      cumpleFecha = cumpleFecha && new Date(v.fecha) >= new Date(filtroFecha.desde);
    }
    if (filtroFecha.hasta) {
      cumpleFecha = cumpleFecha && new Date(v.fecha) <= new Date(filtroFecha.hasta);
    }
  ...
  const totales = {
    totalVentas: ventasFiltradas.reduce((sum, v) => sum + parseFloat(v.total), 0),
    totalCobrado: ventasFiltradas.reduce((sum, v) => sum + (parseFloat(v.monto_pagado) || 0), 0),
    porCobrar: ventasFiltradas.reduce((sum, v) => sum + parseFloat(v.balance_pendiente), 0)
  };
