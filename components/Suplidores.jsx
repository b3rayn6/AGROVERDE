...
export default function Suplidores() {
  ...
  const handleSubmitCuentaPorPagar = async (e) => {
    ...
    await supabase
      .from('cuentas_por_pagar_suplidores')
      .insert([{
        suplidor_id: suplidorSeleccionado.id,
        suplidor_nombre: suplidorSeleccionado.nombre,
        ...formCuentaPorPagar,
        monto: parseFloat(formCuentaPorPagar.monto)
      }]);
  ...
  const handleMarcarComoPagada = async (cuentaId) => {
    if (window.confirm('¿Marcar esta cuenta como pagada?')) {
      await supabase
        .from('cuentas_por_pagar_suplidores')
                    ...
                    <div key={cuenta.id} className={`border rounded-lg p-4 ${cuenta.estado === 'pagada' ? 'bg-green-50 border-green-200' : 'bg-white border-gray-200'}`}>
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          ...
                          <p className="text-sm text-gray-600">Fecha: {new Date(cuenta.fecha_factura).toLocaleDateString()}</p>
                          {cuenta.fecha_vencimiento && (
                            <p className="text-sm text-gray-600">Vence: {new Date(cuenta.fecha_vencimiento).toLocaleDateString()}</p>
                          )}
                        </div>
                        <div className="text-right">
                          <p className="font-bold text-lg text-red-600">{formatCurrency(cuenta.monto)}</p>
                      ...
                      {cuenta.descripcion && (
                        <p className="text-sm text-gray-700 mb-2">{cuenta.descripcion}</p>
                      )}
